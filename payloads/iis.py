...
class IIS_httpsys_rce(BaseWebPlugin):
    bugname = "iis http.sys远程代码执行漏洞"
    bugrank = "高危"
    bugnumber = "CVE-2015-1635|MS15-034"
    bugdesc = "利用HTTP.sys的安全漏洞，攻击者只需要发送恶意的http请求数据包，就可能远程读取IIS服务器的内存数据，或使服务器系统蓝屏崩溃，一定条件下可导致远程代码执行。"
    bugnote = "http://www.myhack58.com/Article/html/3/62/2015/61431.htm"

    def filter(self,web):
        return 'iis' in web.content or 'ASP' in web.xpoweredby
    ...
    def verify(self,web, user='', pwd='',timeout=10):
        try:
            ...
            if b"Requested Range Not Satisfiable" in response and b"Server: nginx" not in response:
